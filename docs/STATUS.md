...
| **GUI 外壳（M4.4）** | ✅ **真机闭环** | C++ webview(WebView2) 宿主 + 纯 HTML/CSS/JS 单页前端（内嵌为字符串），复用引擎+AppRouter；9 个 JS↔C++ bind（listApps/start/stop/capture/setMuted/renameTarget/removeTarget/getStatus/cableStatus）。选 App 下拉(按 PID 去重) + 圈人命名 + 目标名单(相似度条+掐他开关+点名改名+✕删) + 聚合仪表。**真机端到端验通**（chrome 单人闭环）。修过的坑：套间(先 webview 占 STA)、函数名撞绑定(capture→onCapture)、原地更新防点击被吃、自适应自调度轮询防 CPU 雪崩、删目标持锁防悬空。详见 [`exec-plans/m4-app-shell.md`](exec-plans/m4-app-shell.md) 的「M4.4 对齐」。**UI 打磨阶段进行中**（[`exec-plans/ui-polish.md`](exec-plans/ui-polish.md)）：前端正迁移到 **Vue 3 + Naive UI**（U0 管线 `frontend/`→Vite 构建单文件→`embed-frontend.mjs` 嵌入 `kIndexHtml` 已通；U1 复刻功能界面待做，当前 exe 暂为 U0 验证页） | `automute/gui_main.cpp`, `automute/gui_html.cpp`(生成), `frontend/` |
| 声纹录入 / 存储 | ❌ | M2 | — |
