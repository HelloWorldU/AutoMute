...
| M4 | app 壳（产品化 P3+P4+P5）：配置持久化 + enroll UX + GUI | 🚧 **v1 单人闭环真机验通**（M4.1 引擎解耦 / M4.2 多目标+快照 / M4.3 自动路由 / M4.4 GUI 全部✅，含改名删目标等 UX 打磨）。**仅余配置/声纹持久化 → v1.1** |
...
| M4.2 | 引擎扩展：单目标→多目标（按 N 设计，v1=1）+ 暴露"最近 N 秒快照"API（在线抓取登记的地基） | ✅ 引擎扩展完成：多目标列表（聚合 max+per-target getters，任一开静音命中即掐）+ `snapshotRecent()`（抓取线程维护 5s mono 历史环，加锁拷）+ 运行中 `addTarget`。CLI 加热键 c/1-9 冰测；构建通过、快照环形数学单测通过、`--apps` 跑通。真机多目标/在线抓取经 GUI 验过（同人相似度拉开、掐他生效） |
...
| # | 目标 | 状态 |
|---|------|------|
| M3.1 | 分析线程改滑动窗+跳步（不清空重攒）：每 HOP_SEC 对最近 WINDOW_SEC 重判一次，消除"目标横跨窗口被稀释→最坏 ~3s"、尾部更跟手 | ✅ 已接（`app_main.cpp` 调参旋钮 THRESH/WINDOW_SEC/HOP_SEC）；真机体感已由 M4 v1 GUI 闭环覆盖 |
...
| M2.4 | 实时分析线程：抓取 → 分析 ring buffer → 攒窗(1.5s) → Embedder → 余弦比对目标 → 判定(阈值 0.5) | ✅ 程序构建/enroll/双线程跑通；离线管线全验证；真机实时检测已由 M4 v1 GUI 闭环确认 |
| M2.5 | 接线：判定结果驱动 gate，自动掐声 | ✅ 三线程合体(抓取+渲染带gate+分析)，检测目标→自动静音；真机自动掐声已由 M4 v1 GUI 闭环确认 |
...
| **GUI 外壳（M4.4）** | ✅ **真机闭环** | C++ webview(WebView2) 宿主 + 纯 HTML/CSS/JS 单页前端（内嵌为字符串），复用引擎+AppRouter；9 个 JS↔C++ bind（listApps/start/stop/capture/setMuted/renameTarget/removeTarget/getStatus/cableStatus）。选 App 下拉(按 PID 去重) + 圈人命名 + 目标名单(相似度条+掐他开关+点名改名+✕删) + 聚合仪表。**真机端到端验通**（chrome 单人闭环）。修过的坑：套间(先 webview 占 STA)、函数名撞绑定(capture→onCapture)、原地更新防点击被吃、自适应自调度轮询防 CPU 雪崩、删目标持锁防悬空。详见 [`exec-plans/m4-app-shell.md`](exec-plans/m4-app-shell.md) 的「M4.4 对齐」。**UI 打磨阶段**（[`exec-plans/ui-polish.md`](exec-plans/ui-polish.md)）：前端已迁到 **Vue 3 + Naive UI + TS + Vite**（`frontend/`→构建单文件→`scripts/embed-frontend.mjs` 嵌入 `kIndexHtml`）。U0 管线 / U1 复刻功能 / Uw 无边框自绘标题栏(子类化 NCCALCSIZE/NCHITTEST + HTML 缩放手柄) / U2 精致暗色设计语言 / U4 仪表渐变+阈值刻度+动效 全 ✅；起窗口存活验通 | `automute/gui_main.cpp`, `automute/gui_html.cpp`(生成), `frontend/` |
| 声纹录入 / 存储 | ❌ | M2 | — |
...
*产品形态 = P2 进程 loopback + 虚拟声卡路由隔离原声（P1 设备选择已移除）。
**M4 v1 单人直播已真机端到端闭环**（GUI 选 App→自动路由→在线抓取命名→相似度判定→掐声→退出还原）。
下一步 **v1.1**：多人名单（引擎已按 N 设计、GUI 列表已支持多目标，主要是体验细化）+ 配置/声纹**持久化**（免每次重抓）。
更早期遗留可选项：M3.2 VAD / M3.3 阈值迟滞（边际小，暂缓）。详见 [`exec-plans/m4-app-shell.md`](exec-plans/m4-app-shell.md)。*
