...
if (-not (Test-Path third_party\onnxruntime)) {
  Write-Host "[1/5] 下载 ONNX Runtime 1.26.0 ..."
...
} else { Write-Host "[4/5] 声纹模型已存在，跳过" }
...
if (-not (Test-Path third_party\webview\include\webview.h)) {
  Write-Host "[5/5] 下载 webview 0.12.0 单头 ..."
...
} else { Write-Host "[5/5] webview.h 已存在，跳过" }
if (-not (Test-Path third_party\webview\include\WebView2.h)) {
  Write-Host "[5/5] 下载 WebView2 SDK 头（从 nuget 解出）..."
...
} else { Write-Host "[5/5] WebView2.h 已存在，跳过" }
